...
## Historial de sesiones (resumen — detalle completo en `HISTORIAL.md`)

- **2026-09-19**: rediseño visual estilo Uber en las 3 apps (solo
  className/JSX, sin lógica de negocio), fix de `start_url` del manifest
  PWA, GPS activo desde que se abre pasajero/conductor (antes solo al
  pedir viaje / al conectarse).
- **2026-09-19 noche**: botón "instalar app" propio, página pública `/guia`
  con QR, viajes programados + método de pago preferido, ruta real en el
  mapa (OSRM), desglose de tarifa visible, historial de viajes, registro
  real de pasajero/conductor con KYC. Bugs reales corregidos de paso:
  coordenada de Marina Golf Rapel a ~10km de su ubicación real, voseo
  rioplatense en manuales/marketing (corregido a tuteo chileno).
- **2026-09-20**: doc de Postgres vs SQLite arreglada (producción usa
  Postgres real, no Docker); login Google + Teléfono/SMS vía Firebase Auth
  solo para pasajero (RUT opcional), con 9 problemas de seguridad
  encontrados y corregidos antes de tocar producción (revisión cruzada con
  Gemini).
- **Login con Facebook** (antes de 2026-09-23): código pusheado, pero
  pausado por decisión del dueño — ver "Pendientes abiertos" abajo.

## Pendientes abiertos

- **`npx prisma db push` en la Postgres de producción (Railway)** para las
  columnas `Trip.scheduledFor`/`SCHEDULED` y `User.preferredPaymentMethod`
  (agregadas 2026-09-19 noche, hoy solo existen en el schema/preview) —
  requiere aprobación explícita antes de tocar la base de producción.
- **Login con Facebook**: NO es prioridad actual — el dueño lo indicó
  explícitamente. Solo queda `MANUAL_LOGIN_FACEBOOK.md` con los 2 pasos
  manuales (Meta for Developers + consola Firebase) documentados para si
  se retoma en el futuro. No requiere acción ahora.
- **Apple Sign-In**: fuera de alcance — requiere cuenta Apple Developer de
  pago (US$99/año), solo necesario si se publica una app nativa en el App
  Store.
